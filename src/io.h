...
public:
    ...
    {
        mqtt_publish(name, String(get_data()));
        log_msg("Sensor udpate: " + to_string());
    }
...
public:
    periodicSensor(uiElements *ui, const String n, int period) : genSensor(ui, n)
    {
        ticker = new myTicker::Ticker(periodic_wrapper, this, period, 0, myTicker::MILLIS);
    ...
    static void periodic_wrapper(void *a);
    virtual String _to_string(void) override { return String(get_data()); };
    virtual float get_data(void) override = 0;
    virtual void update_data(void) = 0;
    virtual void cb(void)
...
public:
        ...
        : genSensor(ui, n, AVG_SENSOR), sensors(s), act_ind(0)
    {
        ...
        for_each(sensors.begin(), sensors.end(),
                 [&](genSensor *sens) {
                     sens->add_parent(this);
                     /*printf("adding %s as parent to %s\n", get_name().c_str(), sens->get_name().c_str());*/
                 });
    ...
    virtual void update_data(void) override { publish_data(); /*log_msg(name + ": update_data called - shouldn't happen!!!"); */ }
...
public:
    myBM280(uiElements *ui, String n, int sda = 4, int scl = 5, uint8_t address = 0x76, int period = 2000);
    ...
    inline float get_temp()
    {
        ...
        r = temp;
        V(mutex);
        //printf("retrieving temp from sensor %s = %02f\n", get_name().c_str(), r);
        return r;
    }
